...
inline void rb_identify_island(flecs::iter &it, Real dt)
{
	...
	for (size_t j = 0; j < simulation_islands.size(); ++j)
	{
		...
		// Flecs 4.1.1+: get<T>()/get_mut<T>() return references, not pointers
...
inline void rb_clear_collision(flecs::entity e, Constraint &constraint)
{
...
inline void rb_collect_collision(flecs::iter &it)
{
	...
	for (size_t i = 0; i < broad_collision_pairs.size(); ++i)
	{
		...
		// Flecs 4.1.1+: get_mut<T>()/get<T>() return references, not pointers
		...
		colliders_update(
		    colliders1, e1.get<Position>().value, &e1.get_mut<Orientation>().value);
		colliders_update(
		    colliders2, e2.get<Position>().value, &e2.get_mut<Orientation>().value);
