...
inline Vector3 e2r(const Vector3r &v) { return {(float) v.x(), (float) v.y(), (float) v.z()}; }

inline Vector3r e2r(const Vector3 &v) { return {(Real) v.x, (Real) v.y, (Real) v.z}; }
...
int main() {
    ...
    auto clear_force = ecs.system<Forces, LinearForce, AngularForce>("clear force")
                               .with<RigidBody>()
    ...
    auto rigidbody_integrate =
            ecs.system<Position, Orientation, LinearVelocity, AngularVelocity, OldPosition,
                       OldOrientation, const LocalInertia, const LocalInverseInertia,
                       const LinearForce, const AngularForce,
                       const Mass>("integrate")
                    .with<RigidBody>()
                    .without<IsPinned>() // is fixed
                    // .with<IsActive> ()
                    .kind(0)
                    .each([&](flecs::iter &it, size_t, Position &x, Orientation &q,
                              LinearVelocity &v, AngularVelocity &omega, OldPosition &x_old,
                              OldOrientation &q_old, const LocalInertia &I_body,
                              const LocalInverseInertia &I_body_inv, const LinearForce &f,
                              const AngularForce &tau, const Mass mass) {
                        auto dt = it.delta_system_time();
    ...
    auto clear_collision =
            ecs.system<Constraint>("clear collision")
                    .kind(0)
                    .each([](flecs::entity e, Constraint &c) { rb_clear_collision(e, c); });
    ...
    auto update_velocity =
            ecs.system<LinearVelocity, OldLinearVelocity, AngularVelocity, OldAngularVelocity,
                       const Position, const OldPosition, const Orientation, const OldOrientation>(
                       "update velocity")
                    .with<RigidBody>()
                    .kind(0)
                    .each([&](LinearVelocity &v, OldLinearVelocity &v_old, AngularVelocity &omega,
                              OldAngularVelocity &omega_old, const Position &x,
                              const OldPosition &x_old, const Orientation &q,
                              const OldOrientation &q_old) {
                        rb_update_velocity(v.value, v_old.value, omega.value, omega_old.value,
                                           x.value, x_old.value, q.value, q_old.value,
    ...
    ecs.system<const Position, const Orientation, const RenderMesh>("draw rigidbody render mesh")
            .with<RigidBody>()
            .kind(graphics::phase_on_render)
            .each([](flecs::entity e, const Position &x, const Orientation &q,
                     const RenderMesh &render_mesh) {
                auto aa = Eigen::AngleAxisd(q.value);
                DrawModelWiresEx(render_mesh.model, e2r(x.value), e2r(aa.axis()),
                                 aa.angle() * RAD2DEG, e2r(Vector3r::Ones()), BLACK);
            });
    ...
    ecs.system<Constraint>("draw contacts").kind(graphics::phase_on_render).each([](const Constraint &c) {
        if (c.type == phys::pbd::COLLISION_CONSTRAINT) {
            ...
            auto r1_wc =
                    e1.get<Orientation>().value._transformVector(c.collision_constraint.r1_lc) +
                    e1.get<Position>().value;
            auto r2_wc =
                    e2.get<Orientation>().value._transformVector(c.collision_constraint.r2_lc) +
                    e2.get<Position>().value;
