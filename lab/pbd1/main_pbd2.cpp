...
flecs::entity
add_particle(flecs::world& ecs, const Vector3r& pos = Vector3r::Zero(), Real mass = 1.0f) {
    auto e = ecs.entity();
    ...
    e.set<Velocity>({Vector3r::Zero()});
    e.set<Acceleration>({Vector3r::Zero()});

    e.set<Mass>({mass});
    e.set<InverseMass>({1 / mass});

    return e;
}

inline rl::Vector3 e2r(const Vector3r& v) {
    return {(float)v.x(), (float)v.y(), (float)v.z()};
}

inline Vector3r e2r(const rl::Vector3& v) {
...
int main() {
    ...
    graphics::init(ecs);
    graphics::init_window(1280, 720, "muller2007position");

    ecs.set<Scene>({delta_time, 1, 1, gravity});

    auto p1 = add_particle(ecs, Vector3r(0, 4, 0), 1.0f);
    auto p2 = add_particle(ecs, Vector3r(-1, 4, 0), 1.0f);
    auto p3 = add_particle(ecs, Vector3r(-2, 4, 0), 1.0f);
    auto p4 = add_particle(ecs, Vector3r(-3, 4, 0), 1000.0f);
    ...
    add_distance_constraint(ecs, p3, p4, 1e+5);
    ...
    ecs.system<DistanceConstraint>("draw_distance_constraint")
        ...
        .each([](DistanceConstraint& c) {
            ...
            rl::Color color = ColorLerp(rl::RED, rl::GREEN, t);

            // rl::DrawLine3D({x1.x(), x1.y(), x1.z()}, {x2.x(), x2.y(), x2.z()}, rl::GREEN);
    ...
    ecs.system<const Position, const Mass>("draw_particle")
        ...
        .each([](const Position& x, const Mass m) {
            DrawSphere(e2r(x.value), 0.05 * std::pow(m.value, 1.0f / 3.0f), rl::BLUE);
            // rl::DrawSphereWires(e2r(x.value), 0.05 * std::pow(m.value, 1.0f / 3.0f), 16, 16, rl::BLUE);
        });
    ...
    ecs.system<const Position, const Mass>("draw mass info")
        ...
        .each([](const Position& x, const Mass& m) {
            rl::Vector3 textPos = e2r(x.value);
            textPos.y += 0.2f;
            rl::Vector2 screenPos = GetWorldToScreen(textPos, graphics::camera1);

            char id_buffer[32];

            const char* label = (sprintf(id_buffer, "%0.2f", m.value), id_buffer);

            DrawText(label, screenPos.x, screenPos.y, 20, rl::BLUE);
        });
    ...
    ecs.system<DistanceConstraint>("draw_constraint_lambda")
        ...
        .each([](DistanceConstraint& c) {
            ...
            rl::DrawText(label, screenPos.x, screenPos.y, 20, rl::BLUE);
        });
    ...
    ecs.system("you can do it").kind(graphics::PostRender).run([](flecs::iter& it) {
        ...
        rl::DrawText(buffer, 20, 100, 20, rl::DARKGREEN);
    });
