...
int main() {
    ...
    ecs.system<Position>("draw_particle")
        ...
        .each([](Position& x) {
            // rl::DrawSphere({x.value.x(), x.value.y(), x.value.z()}, 0.05, rl::BLUE);
            rl::DrawPoint3D({x.value.x(), x.value.y(), x.value.z()}, rl::BLUE);
        });
    ...
    ecs.system("stats").kind(graphics::PostRender).run([&](flecs::iter& it) {
        ...
        char buffer[64];
        // clang-format off
        rl::DrawText((sprintf(buffer, "Elapsed time: %.2f", params->elapsed), buffer),20, 100, 20,rl::DARKGREEN);
        rl::DrawText((sprintf(buffer, "Particles: %d", particle_count), buffer), 20, 130, 20, rl::DARKGREEN);
        rl::DrawText((sprintf(buffer, "Constraints: %d", constraint_count), buffer), 20, 160, 20, rl::DARKGREEN);
        // clang-format on
    });
