...
int main() {
    ...
    rb2.add<BVH>();
    // rb2.add<IsPinned>();
    rb2.set<Friction>({0.0f});

    auto rb3 = rb_add_rigid_body(ecs, 1);
    rb3.add<Geometry>();
    rb3.add<AABB>();
    rb3.add<BVH>();
    rb3.set<Friction>({0.1f});
    ...
    rb5.add<AABB>();
    rb5.add<BVH>();
    rb5.set<Friction>({0.3f});

    rl::Mesh mesh1 = rl::GenMeshCube(10, 0.2, 10);
    ...
    rb2.get_mut<Position>()->value = {-1, 1, -4};
    rb2.get_mut<LinearVelocity>()->value = {0, 0, 2};

    rb3.get_mut<Position>()->value = {0, 1, -4};
    rb3.get_mut<LinearVelocity>()->value = {0, 0, 2};

    rb4.get_mut<Position>()->value = {1, 1, -4};
    rb4.get_mut<LinearVelocity>()->value = {0, 0, 2};

    rb5.get_mut<Position>()->value = {2, 1, -4};
    rb5.get_mut<LinearVelocity>()->value = {0, 0, 2};
