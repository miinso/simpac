...
int main() {
    ...
    auto rb6 = rb_add_rigid_body(ecs, 1);
    rb6.add<Geometry>();
    rb6.add<AABB>();
    rb6.add<BVH>();
    rb6.set<Friction>({1.0f});

    rl::Mesh mesh1 = rl::GenMeshCube(8, 0.2, 8);
    init_geometry_from_rlmesh(rb1, mesh1, 1.0f);
    rl::UnloadMesh(mesh1);

    rl::Mesh mesh2 = rl::GenMeshCube(0.2, 1, 0.2);
    init_geometry_from_rlmesh(rb2, mesh2, 0.1f);
    rl::UnloadMesh(mesh2);

    rl::Mesh mesh3 = rl::GenMeshCube(0.2, 1, 0.2);
    init_geometry_from_rlmesh(rb3, mesh3, 0.1f);
    rl::UnloadMesh(mesh3);

    rl::Mesh mesh4 = rl::GenMeshCube(0.2, 1, 0.2);
    init_geometry_from_rlmesh(rb4, mesh4, 0.1f);
    rl::UnloadMesh(mesh4);

    rl::Mesh mesh5 = rl::GenMeshCube(0.2, 1, 0.2);
    init_geometry_from_rlmesh(rb5, mesh5, 0.1f);
    rl::UnloadMesh(mesh5);

    rl::Mesh mesh6 = rl::GenMeshCube(3, 0.2, 3);
    init_geometry_from_rlmesh(rb6, mesh6, 0.1f);
    rl::UnloadMesh(mesh6);

    rb2.get_mut<Position>()->value = {-1, 1, -1};
    rb3.get_mut<Position>()->value = {-1, 1, 1};
    rb4.get_mut<Position>()->value = {1, 1, -1};
    rb5.get_mut<Position>()->value = {1, 1, 1};
    rb6.get_mut<Position>()->value = {0, 2, 0};
