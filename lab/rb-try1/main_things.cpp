...
int main() {
    flecs::world ecs;
    // ecs.set_target_fps(120);

    graphics::init(ecs);
    graphics::init_window(800, 800, "Physics Simulator");

    auto mat1 = create_geometry_material();
    ...
    rl::Mesh mesh3 = rl::GenMeshSphere(1, 16, 16);
    init_geometry_from_rlmesh(rb3, mesh3, 1.0f);
    rl::UnloadMesh(mesh3);

    rl::Mesh mesh4 = rl::GenMeshCylinder(0.3, 1, 16);
    init_geometry_from_rlmesh(rb4, mesh4, 1.0f);
    rl::UnloadMesh(mesh4);

    rl::Mesh mesh5 = rl::GenMeshHemiSphere(1, 16, 16);
    init_geometry_from_rlmesh(rb5, mesh5, 1.0f);
    rl::UnloadMesh(mesh5);

    // rb1.get_mut<AngularVelocity>()->value = {0.0, 0.5, 0.0};

    rb2.get_mut<Position>()->value = {3, 2, 0.2};
    // rb2.get_mut<AngularVelocity>()->value = {0.0, 1.2, 0.0};

    rb3.get_mut<Position>()->value = {0, 4, 0};
    // rb3.get_mut<AngularVelocity>()->value = {0.0, -1.2, 0.0};

    rb4.get_mut<Position>()->value = {0, 3, 3};
    rb4.get_mut<Orientation>()->value = Eigen::Quaternionf(Eigen::AngleAxisf(3.14 / 2, Eigen::Vector3f::UnitX()));

    rb5.get_mut<Position>()->value = {-3, 2, 0};
    rb5.get_mut<Orientation>()->value = Eigen::Quaternionf(Eigen::AngleAxisf(3.14 / 2, Eigen::Vector3f::UnitX()));
    ...
    ecs.system("HandleKeypress").kind(flecs::PreUpdate).run([](flecs::iter& it) {
        it.world().each([&](BVH& bvh) {
            if (IsKeyPressed(rl::KEY_APOSTROPHE)) {
                bvh.render_depth = -1;
            } else if (IsKeyPressed(rl::KEY_ZERO)) {
                bvh.render_depth = 0;
            } else if (IsKeyPressed(rl::KEY_ONE)) {
                bvh.render_depth = 1;
            } else if (IsKeyPressed(rl::KEY_TWO)) {
                bvh.render_depth = 2;
            } else if (IsKeyPressed(rl::KEY_THREE)) {
                bvh.render_depth = 3;
            } else if (IsKeyPressed(rl::KEY_FOUR)) {
                bvh.render_depth = 4;
            } else if (IsKeyPressed(rl::KEY_FIVE)) {
                bvh.render_depth = 5;
            } else if (IsKeyPressed(rl::KEY_SIX)) {
                bvh.render_depth = 6;
            } else if (IsKeyPressed(rl::KEY_SEVEN)) {
                bvh.render_depth = 7;
            } else if (IsKeyPressed(rl::KEY_EIGHT)) {
                bvh.render_depth = 8;
            }
        });
    });

    ecs.system<const BVH>("draw_bvh").kind(graphics::OnRender).each([](const BVH& bvh) {
        if (!bvh.initialized)
            return;
        ...
        auto predicate = [&](unsigned int node_index, unsigned int depth) { return (int)depth <= bvh.render_depth; };
