...
int main() {
    ...
    // rb1.get_mut<AngularVelocity>()->value = {0.3, 0, 1};
    ...
    rb2.get_mut<Position>()->value = {1.2, 0, 0};
    ...
    ecs.system<CollisionPair>().each([](flecs::iter& it, size_t, CollisionPair& pair) {
        ...
        auto cb = [&](unsigned int node_index, unsigned int depth) {
            ...
            for (auto i = node.begin; i < node.begin + node.n; ++i) {
                ...
                if (co2->collision_test(x, tolerance, cp, n, dist)) {
                    ...
                    // std::cout << "[narrowphase] it's a hit! center hit!" << std::endl;
        ...
        // bvh1->bvh_.traverse_depth_first(TraversalPredicate pred, TraversalCallback cb)
    });
