...
int main() {
    ...
    rl::Mesh mesh2 = rl::GenMeshCylinder(0.3, 1, 16);
    init_geometry_from_rlmesh(rb2, mesh2, 1.0f);
    rl::UnloadMesh(mesh2);

    rl::Mesh mesh3 = rl::GenMeshCylinder(0.3, 1, 16);
    init_geometry_from_rlmesh(rb3, mesh3, 1.0f);
    rl::UnloadMesh(mesh3);

    rb2.get_mut<Position>()->value = {-3, 1, 0};
    rb2.get_mut<Orientation>()->value = Eigen::Quaternionf(Eigen::AngleAxisf(3.14 / 2, Eigen::Vector3f::UnitX()));
    rb2.get_mut<AngularVelocity>()->value = {1, 0, 0};

    rb3.get_mut<Position>()->value = {3, 1, 0};
    rb3.get_mut<Orientation>()->value = Eigen::Quaternionf(Eigen::AngleAxisf(3.14 / 2, Eigen::Vector3f::UnitX()));
    rb3.get_mut<AngularVelocity>()->value = {0, 5, 0};
