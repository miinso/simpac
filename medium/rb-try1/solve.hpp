...
void compute_rhs(Contact& contact, Vector3r& b, float inv_dt) {
    ...
    if (!contact.e1.has<IsPinned>()) {
        const auto& vel1 = contact.e1.get<LinearVelocity>();
        const auto& omega1 = contact.e1.get<AngularVelocity>();
    ...
    if (!contact.e2.has<IsPinned>()) {
        const auto& vel2 = contact.e2.get<LinearVelocity>();
        const auto& omega2 = contact.e2.get<AngularVelocity>();
