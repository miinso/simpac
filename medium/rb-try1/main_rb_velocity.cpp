...
int main() {
    ...
    rb2.get_mut<Position>().value = {2, 0, 0};
    rb2.get_mut<LinearVelocity>().value = {-0.1, 0, 0};
    // rb2.get_mut<AngularVelocity>().value = {0.0, 0.0, 0.3};

    // create raylib mesh
    Mesh mesh1 = GenMeshSphere(1, 16, 16);
    ...
    ecs.system<LinearForce, const Mass>("apply_gravity")
        .with<RigidBody>()
        .without<IsPinned>()
    ...
    ecs.system<AABB, Geometry>("update_aabb")
        .with<RigidBody>()
        .each([](AABB& aabb, const Geometry& geom) {
    ...
    ecs.system<BVH, Geometry>("update_bvh")
        .with<RigidBody>()
        .each([](BVH& bvh, const Geometry& geom) {
            if (!bvh.initialized) {
        ...
        auto cb = [&](unsigned int node_index, unsigned int depth) {
            ...
            for (auto i = node.begin; i < node.begin + node.n; ++i) {
                ...
                if (co2.collision_test(x, tolerance, cp, n, dist)) {
                    ...
                    it.world().entity().set<Contact>(
                        {rb1, rb2, x_w, cp_w, n_w, dist, restitution, friction});
                }
            }
        };
    ...
    ecs.system<ConstraintForce, ConstraintTorque>().with<RigidBody>().each(
        rb_clear_constraint_forces);
    ...
    ecs.system<const Contact>("draw_contact")
        ...
        .each([](const Contact& contact) {
            ...
            DrawLine3D({(float)cp.x(), (float)cp.y(), (float)cp.z()},
                       {(float)normal_end.x(), (float)normal_end.y(), (float)normal_end.z()},
                       BLUE);
