...
void init_collider_from_rlmesh(flecs::entity entity, const Mesh& rlMesh) {
    if (!entity.has<Collider>()) {
        entity.add<Collider>();
    }
    auto& collider = entity.get_mut<Collider>();

    // rlMesh에서 직접 데이터 읽어서 SDF 생성
    Eigen::Matrix<unsigned int, 3, 1> resolution{20, 20, 20};

    std::vector<Vector3r> vertices(rlMesh.vertexCount);
    for(int i = 0; i < rlMesh.vertexCount; i++) {
        vertices[i] = Vector3r(rlMesh.vertices[i*3],
                             rlMesh.vertices[i*3+1],
                             rlMesh.vertices[i*3+2]);
    }
...
void init_collider_from_geometry(flecs::entity entity) {
    if (!entity.has<Collider>()) {
        entity.add<Collider>();
    }
    ...
    auto& collider = entity.get_mut<Collider>();
    const auto& geom = entity.get<Geometry>();

    // geometry의 데이터를 사용해서 SDF 생성
    Eigen::Matrix<unsigned int, 3, 1> resolution{20, 20, 20};
