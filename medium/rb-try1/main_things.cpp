...
int main() {
    ...
    Mesh mesh3 = GenMeshSphere(1, 16, 16);
    geom_init_geometry_from_rlmesh(rb3, mesh3, 1.0f);
    init_collider_from_geometry(rb3);
    UnloadMesh(mesh3);

    Mesh mesh4 = GenMeshCylinder(0.3, 1, 16);
    geom_init_geometry_from_rlmesh(rb4, mesh4, 1.0f);
    init_collider_from_geometry(rb4);
    UnloadMesh(mesh4);

    Mesh mesh5 = GenMeshHemiSphere(1, 16, 16);
    geom_init_geometry_from_rlmesh(rb5, mesh5, 1.0f);
    ...
    ecs.system<const BVH>("draw_bvh").kind(graphics::phase_on_render).each([](const BVH& bvh) {
