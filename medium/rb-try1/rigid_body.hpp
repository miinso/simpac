...
void draw_rb_debug_text(
    ...
    // position
    DrawText(("Position: " + vec3_to_string(pos.value)).c_str(),
                 x, y, font_size, text_color);
    y += line_spacing;

    // orientation
    DrawText(("Orientation: " + quat_to_string(orient.value)).c_str(),
                 x, y, font_size, text_color);
    y += line_spacing;

    // linear velocity
    DrawText(("Linear Velocity: " + vec3_to_string(vel.value)).c_str(),
                 x, y, font_size, text_color);
    y += line_spacing;
