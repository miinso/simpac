...
static void EnableWebGLExtensions(void)
{
#if defined(__EMSCRIPTEN__)
    // enable webgl extension for float textures
    ...
    // enable EXT_color_buffer_float extension (required for floating point textures as render targets)
    if (emscripten_webgl_enable_extension(context, "EXT_color_buffer_float") != 1) {
        TraceLog(LOG_WARNING, "Failed to enable EXT_color_buffer_float extension");
    } else {
        TraceLog(LOG_INFO, "Successfully enabled EXT_color_buffer_float extension");
    }
#endif
}
...
int main(void)
{
    ...
    Shader gbufferShader = LoadShader(TextFormat("resources/shaders/glsl%s/gbuffer.vs", GLSL_VERSION),
                               TextFormat("resources/shaders/glsl%s/gbuffer.fs", GLSL_VERSION));

    Shader deferredShader = LoadShader(TextFormat("resources/shaders/glsl%s/deferred_shading.vs", GLSL_VERSION),
                               TextFormat("resources/shaders/glsl%s/deferred_shading.fs", GLSL_VERSION));
    ...
    gBuffer.positionTexture = rlLoadTexture(NULL, screenWidth, screenHeight, RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16A16, 1);
    ...
    gBuffer.normalTexture = rlLoadTexture(NULL, screenWidth, screenHeight, RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16A16, 1);
    ...
    DeferredMode mode = DEFERRED_POSITION;
