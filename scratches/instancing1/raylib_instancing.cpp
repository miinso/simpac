...
int main() {
    ...
    Shader shader =
        LoadShader(TextFormat("resources/shaders/glsl%s/pbr_instancing.vs", GLSL_VERSION),
    ...
    Mesh boxMesh = GenMeshSphere(0.2, 60, 60);
    ...
    ecs.system("draw 3d").kind(graphics::OnRender).run([&](flecs::iter& it) {
        ...
        // DrawMesh(box.meshes[0], box.materials[0], box.transform);

        // Matrix baseTransform = GizmoToMatrix(boxTransform);
        ...
        //         for (int z = 0; z < 4; z++) {
        //             Matrix offset =
        //                 MatrixTranslate((float)x * spacing, (float)y * spacing, (float)z * spacing);

        //             transforms[index] = baseTransform; //MatrixMultiply(offset, baseTransform);
        //             index++;
        //         }
        //     }
        // }

        Matrix* transforms = static_cast<Matrix*>(
            RL_CALLOC(4, sizeof(Matrix))); // Pre-multiplied transformations passed to rlgl
        // Translate and rotate cubes randomly
        for (int i = 0; i < 4; i++) {
            Matrix translation = MatrixTranslate(static_cast<float>(GetRandomValue(-2, 2)),
                                                 static_cast<float>(GetRandomValue(-2, 2)),
                                                 static_cast<float>(GetRandomValue(-2, 2)));

            Vector3 axis = Vector3Normalize(Vector3{static_cast<float>(GetRandomValue(0, 360)),
        ...
        DrawMeshInstanced(box.meshes[0], box.materials[0], transforms, 4);
