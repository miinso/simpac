...
void check_particle_rest_state(flecs::iter& it, size_t index, Position& p, Velocity& v, ParticleProperties& props) {
    auto scene = it.world().get<Scene>();
...
void reset_particles(flecs::iter& it) {
    ...
    auto add_plane = [&](const Eigen::Vector3f& pos, const Eigen::Vector3f& normal, const bool visible = true) {
        it.world().entity().add<Plane>().set<Position>({pos}).set<PlaneProperties>({normal, 0.3f, 0.7f, 0, visible});
    };
...
int main() {
    ...
    // Create entities
    auto add_particle = [&](const Eigen::Vector3f& pos, const Eigen::Vector3f& vel) {
        ecs.entity().add<Particle>().set<Position>({pos}).set<Velocity>({vel}).set<ParticleProperties>(
            {0.5f, 1.0f, 0.3f, 0.7f, false});
    };
    ...
    auto add_plane = [&](const Eigen::Vector3f& pos,
        ...
        ecs.entity().add<Plane>().set<Position>({pos}).set<PlaneProperties>({normal, 0.3f, 0.7f, 0, visible, audible});
    };
    ...
    auto simulate_particles_system = ecs.system<Position, Velocity, const ParticleProperties>().each(
    ...
    auto check_particle_rest_state_system = ecs.system<Position, Velocity, ParticleProperties>().each(
        [](flecs::iter& it, size_t index, Position& p, Velocity& v, ParticleProperties& props) {
    ...
    ecs.system<const Position, const PlaneProperties>("DrawPlanes").kind(flecs::PostUpdate).each(draw_planes);
    ...
    ecs.system("HandleTimeStepControl").kind(flecs::PostUpdate).run([](flecs::iter& it) {
        ...
        float logValue = LinearToLog(scene->frequency_slider, scene->min_frequency, scene->max_frequency);
