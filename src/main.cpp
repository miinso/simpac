...
void SpawnEnemy(flecs::iter& it, const Game* g)
{
	...
	it.world()
		...
		.is_a<prefabs::Enemy>()
		.set<Direction>({0})
		.set<Position>({lvl->spawn_point.x, 1.2, lvl->spawn_point.y});
}

void MoveEnemy(flecs::iter& it, size_t i, Position& p, Direction& d, const Game& g)
{
	const Level* lvl = g.level.get<Level>();
...
void FindTarget(flecs::iter& it, size_t i, Turret& turret, Target& target, Position& p)
{
...
void AimTarget(flecs::iter& it, size_t i, Turret& turret, Target& target, Position& p)
{
...
void FireCountdown(flecs::iter& it, size_t i, Turret& turret, Target& target)
{
	turret.t_since_fire += it.delta_time();
}

void FireAtTarget(flecs::iter& it, size_t i, Turret& turret, Target& target, Position& p)
{
	auto ecs = it.world();
	...
	if(target.target && target.lock)
	{
		...
		if(!is_laser)
		{
			...
			ecs.entity().is_a<prefabs::Bullet>().set<Position>(pos).set<Velocity>(
				{-v[0], 0, -v[2]});
			ecs.entity().is_a<prefabs::NozzleFlash>().set<Position>(pos).set<Rotation>(
				{0, angle, 0});
		}
		else
		{
			...
			pos.y = 1.1;
			pos.z += 1.4 * -v[2];
			ecs.scope<particles>().entity().is_a<prefabs::Bolt>().set<Position>(pos).set<Rotation>(
				{0, angle, 0});
		}
...
void BeamControl(flecs::iter& it, size_t i, Position& p, Turret& turret, Target& target)
{
	...
	if(target.lock && beam && beam.enabled())
	{
		...
		if(randf(1.0) > 0.5)
		{
			...
			Velocity ion_v = {randf(0.02), randf(0.02) + 0.01f, randf(0.02)};
...
void ProgressParticle(
	flecs::iter& it, ParticleLifespan* pl, const Particle* p, Box* box, Color* color, Velocity* vel)
{
	if(it.is_set(3))
	{ // Size
		for(auto i : it)
...
void HitTarget(flecs::iter& it, size_t i, Position& p, Health& h, Box& b, HitCooldown& hit_cooldown)
{
...
static void explode(flecs::world& ecs, Position& p)
{
	...
	for(int s = 0; s < SmokeParticleCount; s++)
	{
		...
		ecs.scope<particles>()
			.entity()
			.is_a<prefabs::Smoke>()
			.set<Position>({p.x + randf(ExplodeRadius) - ExplodeRadius / 2,
	...
	for(int s = 0; s < SparkParticleCount; s++)
	{
		...
		ecs.scope<particles>()
			...
			.set<Velocity>({cos(x_r) * speed, cos(y_r) * speed, cos(z_r) * speed});
	}
}
...
void init_ui(flecs::world& ecs)
{
	...
	auto light = ecs.entity("Sun").set<graphics::DirectionalLight>(light_data);
...
void init_level(flecs::world& ecs)
{
	...
	transform::Position2 spawn_point = {to_x(TileCountX - 1), to_z(TileCountZ - 1)};
...
void init_prefabs(flecs::world& ecs)
{
	...
	ecs.prefab<prefabs::materials::Beam>().set<Color>({0.1, 0.4, 1}).set<Emissive>({10.0});

	ecs.prefab<prefabs::materials::Metal>().set<Color>({0.1, 0.1, 0.1}).set<Specular>({1.5, 128});

	ecs.prefab<prefabs::materials::CannonHead>()
		.set<Color>({0.35, 0.4, 0.3})
		.set<Specular>({0.5, 25});

	ecs.prefab<prefabs::materials::LaserLight>().set<Color>({0.1, 0.3, 1.0}).set<Emissive>({3.0});
	...
	ecs.prefab<prefabs::Particle>().override<ParticleLifespan>().override<Color>().override<Box>();

	ecs.prefab<prefabs::Bullet>()
		.is_a<prefabs::Particle>()
		.add<Bullet>()
		.set<Color>({0, 0, 0})
		.set<Box>({BulletSize, BulletSize, BulletSize})
		.set<Particle>({1.0, 1.0, 1.0, BulletLifespan});

	ecs.prefab<prefabs::NozzleFlash>()
		.is_a<prefabs::Particle>()
		.set<Color>({1.0, 0.5, 0.3})
		.set<Emissive>({5.0})
		.set<Box>({NozzleFlashSize, NozzleFlashSize, NozzleFlashSize})
		.set<Particle>({NozzleFlashSizeDecay, 1.0, 1.0, NozzleFlashLifespan});

	ecs.prefab<prefabs::Smoke>()
		.is_a<prefabs::Particle>()
		.set<Color>({0, 0, 0})
		.set<Emissive>({20.0})
		.set<Box>({SmokeSize, SmokeSize, SmokeSize})
		.set<Particle>({SmokeSizeDecay, SmokeColorDecay, 1.0, SmokeLifespan})
		.set<Velocity>({0, 0.8, 0})
		.override<Velocity>();

	ecs.prefab<prefabs::Spark>()
		.is_a<prefabs::Particle>()
		.set<Color>({1.0, 0.5, 0.2})
		.set<Emissive>({5.0})
		.set<Box>({SparkSize, SparkSize, SparkSize})
		.set<Particle>({SparkSizeDecay, 1.0, SparkVelocityDecay, SparkLifespan});

	ecs.prefab<prefabs::Ion>()
		.is_a<prefabs::materials::Beam>()
		.is_a<prefabs::Particle>()
		.set<Box>({IonSize, IonSize, IonSize})
		.set<Particle>({IonDecay, 1.0, 1.0, IonLifespan});

	ecs.prefab<prefabs::Bolt>()
		.is_a<prefabs::materials::Beam>()
		.is_a<prefabs::Particle>()
		.set<Box>({BoltSize, BoltSize, BoltSize})
		.set<Particle>({BoltSizeDecay, 1.0, 1.0, BoltLifespan});
	...
	ecs.prefab<prefabs::Turret::Base>().slot().set<Position>({0, 0, 0});
	...
	ecs.prefab<prefabs::Cannon>().is_a<prefabs::Turret>().set<Turret>({TurretFireInterval});
	...
	ecs.prefab<prefabs::Cannon::Barrel>().is_a<prefabs::materials::Metal>().set<Box>(
		{0.8, 0.14, 0.14});
	...
	ecs.prefab<prefabs::Laser>().is_a<::prefabs::Turret>().add<Laser>().set<Turret>(
		{BeamFireInterval});

	ecs.prefab<prefabs::Laser::Head>().set<Position>({0.0, 0.8, 0}).set<Rotation>({0, 0.0, 0});
	...
	ecs.prefab()
		.is_a<prefabs::materials::LaserLight>()
		.child_of<prefabs::Laser::Head>()
		.set<Box>({0.8, 0.2, 0.1})
	...
	ecs.prefab<prefabs::Laser::Head::Beam>()
		.slot_of<prefabs::Laser>()
		.is_a<prefabs::materials::Beam>()
		.set<Position>({2, 0, 0})
		.set<Box>({0})
		.override<Box>()
...
void init_systems(flecs::world& ecs)
{
	ecs.scope<systems>([&]() { // Keep root scope clean
		...
		ecs.system<Target, Position>("ClearTarget").each(ClearTarget);

		// Find target for turrets
		ecs.system<Turret, Target, Position>("FindTarget")
			.term<SpatialQuery, Enemy>()
			.up()
			.term<SpatialQueryResult, Enemy>()
			.each(FindTarget);

		// Aim turret at enemies
		ecs.system<Turret, Target, Position>("AimTarget").each(AimTarget);

		// Countdown until next fire
		ecs.system<Turret, Target>("FireCountdown").each(FireCountdown);

		// Aim beam at target
		ecs.system<Position, Turret, Target>("BeamControl").each(BeamControl);
		...
		ecs.system<Position, const Recoil>("ApplyRecoil").each(ApplyRecoil);

		// Decrease recoil amount over time
		ecs.system<Recoil>("DecreaseRecoil").each(DecreaseRecoil);

		// Decrease recoil amount over time
		ecs.system<HitCooldown>("DecreaseHitCooldown").each(DecreaseHitCoolDown);

		// Simple particle system
		ecs.system<ParticleLifespan, const Particle, Box, Color, Velocity>("ProgressParticle")
			.term_at(2)
			.up() // shared particle properties
			.term_at(3)
			.optional()
			.term_at(4)
			.optional()
		...
		ecs.system<Position, Health, Box, HitCooldown>("HitTarget")
			.term<SpatialQuery, Bullet>()
			.up()
			.term<SpatialQueryResult, Bullet>()
			.each(HitTarget);

		// Destroy enemy when health goes to 0
		ecs.system<Health, Position>("DestroyEnemy").term<Enemy>().each(DestroyEnemy);

		// Update enemy color when hit & when health decreases
		ecs.system<Color, Health, const HitCooldown>("UpdateEnemyColor").each(UpdateEnemyColor);
	});
}
...
int main(int argc, char* argv[])
{
	...
	ecs.app().enable_rest().target_fps(60).run();
}
