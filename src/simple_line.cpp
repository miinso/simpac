...
void init_ui(flecs::world& ecs)
{
	...
	auto light = ecs.entity("DirectionalLight").set<graphics::DirectionalLight>(light_data);
...
void init_prefabs(flecs::world& ecs)
{
	...
	ecs.prefab<prefabs::Edge>().is_a<prefabs::materials::Metal>().set<geometry::Line3>(
		{0, 0, 0, 1, 1, 1});
}
...
void init_systems(flecs::world& ecs)
{
	ecs.scope<systems>([&]() { // Keep root scope clean
		ecs.system<Velocity, const Gravity>("Gravity").term<Actor>().term_at(2).singleton().each(
			[](flecs::iter& it, size_t, Velocity& v, const Gravity& g) {
		...
		ecs.system<Position, Velocity>("Bottom").term<Actor>().each(
			[](flecs::iter& it, size_t, Position& p, Velocity& v) {
				if((p.y) < 0)
				{
					p.y = 5;
					v.y = 0;
...
int main(int argc, char* argv[])
{
	...
	ecs.entity("rb").is_a<prefabs::Enemy>().set<Position>({0, 1.2, 0}).set<Velocity>({0, 0, 0});

	ecs.entity("floor").is_a<prefabs::Plane>().set<Position>({0, 0, 0});

	ecs.entity("line1")
		.is_a<prefabs::materials::Metal>()
		.set<geometry::Line3>({1, 0, 0, 1, 1, 1})
		.set<geometry::Stroke>({5.5, 1, 1, 0})
		.set<Position>({0, 2, 0});

	ecs.entity("circle")
		.is_a<prefabs::materials::Metal>()
		.set<geometry::Rectangle>({1, 1})
		.set<Position>({0, 1, 0});

	ecs.app().enable_rest().target_fps(10).run();
}