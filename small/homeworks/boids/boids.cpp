// Boid Simulator
// Explicit system execution with .run() calls
...
int main() {
    ...
    auto handle_keypress = ecs.system("HandleKeypress")
        .kind(0)  // Not part of any pipeline
    ...
    auto find_neighbors_spatial = ecs.system<const Position, NearbyBoids>("FindNeighborsSpatial")
        .kind(0)
        .with<Boid>()
        .each(systems::find_neighbors_spatial);

    auto find_neighbors_brute = ecs.system<const Position, NearbyBoids>("FindNeighborsBrute")
        .kind(0)
        .with<Boid>()
        .each(systems::find_neighbors_brute);

    auto reset_acceleration = ecs.system<Acceleration>("ResetAcceleration")
        .kind(0)
        .with<Boid>()
        .each([](Acceleration& acc) { acc.value.setZero(); });

    auto separation = ecs.system<const Position, const Velocity, const MaxSpeed, const MaxForce, Acceleration, const NearbyBoids>("Separation")
        .kind(0)
        .with<Boid>()
    ...
    auto alignment = ecs.system<const Position, const Velocity, const MaxSpeed, const MaxForce, Acceleration, const NearbyBoids>("Alignment")
        .kind(0)
        .with<Boid>()
        .each([](flecs::entity e, const Position& pos, const Velocity& vel, const MaxSpeed& max_speed,
                 const MaxForce& max_force, Acceleration& acc, const NearbyBoids& nearby) {
    ...
    auto cohesion = ecs.system<const Position, const Velocity, const MaxSpeed, const MaxForce, Acceleration, const NearbyBoids>("Cohesion")
        .kind(0)
        .with<Boid>()
        .each([](flecs::entity e, const Position& pos, const Velocity& vel, const MaxSpeed& max_speed,
                 const MaxForce& max_force, Acceleration& acc, const NearbyBoids& nearby) {
    ...
    auto predator_avoidance = ecs.system<const Position, Acceleration, const NearbyBoids>("PredatorAvoidance")
        .kind(0)
        .with<Boid>()
    ...
    auto obstacle_avoidance = ecs.system<const Position, Acceleration, const NearbyBoids>("ObstacleAvoidance")
        .kind(0)
        .with<Boid>()
        .each([](flecs::entity e, const Position& pos, Acceleration& acc, const NearbyBoids& nearby) {
            const auto& params = e.world().get<FlockingParams, Boid>();
    ...
    auto predator_flocking = ecs.system<const Position, const Velocity, const MaxSpeed, const MaxForce, Acceleration>("PredatorFlocking")
        .kind(0)
        .with<Predator>()
        .each([](flecs::entity e, const Position& pos, const Velocity& vel, const MaxSpeed& max_speed,
    ...
    auto update_movement = ecs.system<Position, Velocity, Acceleration, const MaxSpeed>("UpdateMovement")
        .kind(0)
        .each(systems::update_movement);

    // =========================================================================
    // Rendering systems (keep on graphics phases - run via ecs.progress())
    // =========================================================================
    ...
    ecs.system("BoidSimulation")
        ...
        .run([&](flecs::iter& it) {
            ...
            // Neighbor finding (conditional based on debug settings)
            const auto& debug = it.world().get<DebugSettings>();
            ...
            alignment.run(dt);
            cohesion.run(dt);
            predator_avoidance.run(dt);
            obstacle_avoidance.run(dt);

            // Predator behavior
            predator_flocking.run(dt);

            // Physics integration
            update_movement.run(dt);
        });
